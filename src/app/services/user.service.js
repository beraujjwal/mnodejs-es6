...
import * as redisClient from '../../libraries/redis.library';
const tokenService = new token('Token');
const roleService = new role('Role');
import userGraph from '../../neo4j/services/user';

import { sentOTPMail } from '../../libraries/email.library';
import { sentOTPSMS } from '../../libraries/sms.library';
...
class user extends service {
  ...
  constructor(model) {
    ...
    this.role = this.models['Role'];
    this.token = this.models['Token'];
  ...
  async login(
    ...
    try {
      ...
      if (!user) {
        throw new baseError(
            'We are unable to find your account with the given credentials.', 401
        );
      }
  ...
  async invalidLoginAttempt(user) {
    try {
      ...
      if (loginAttempts >= blockLoginAttempts) {
        throw new baseError('Your login attempts exist. Please try after 300 seconds.');
      } else {
