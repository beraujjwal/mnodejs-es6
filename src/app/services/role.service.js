...
import { validationError } from '../../system/core/error/validationError';
import roleGraph from '../../neo4j/services/role';
...
class role extends service {
  ...
  constructor(model) {
    ...
    this.resource = this.models['Resource'];
    //autoBind(this);
  }
  ...
  async checkUserRoleAvailablity( {roles, parentRole, defaultRole = 'admin'}, session ){
    try{
      ...
      let dbRoles = await this.db['Role'].findOne({ slug: { $in: roles }});/*.populate({
        path: 'role',
        match: {
          slug: parentRole
        }
